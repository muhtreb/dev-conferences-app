...
class EditionsController extends AbstractController
{
    ...
    private function generateUrls(ApiClient $apiClient): \Generator
    {
        $page = 1;
        do {
            ['data' => $editions, 'meta' => $meta] = $apiClient->getEditions(limit: 100, page: $page);
            foreach ($editions as $edition) {
                yield '<url>' . PHP_EOL;
                yield '<loc>' . htmlspecialchars($this->generateUrl('edition_show', ['slug' => $edition['slug']], UrlGeneratorInterface::ABSOLUTE_URL)) . '</loc>' . PHP_EOL;
